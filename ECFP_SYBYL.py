...
def test(input_file, epochs, property, n_splits, normalize):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(data)):
        x_train = np.concatenate((molecules(data['smiles'][train_index].tolist()).ECFP(),molecules(data['smiles'][train_index].tolist()).SYBYL()), axis=1)
        y_train = data[property].iloc[train_index].values
        
        x_test = np.concatenate((molecules(data['smiles'][test_index].tolist()).ECFP(),molecules(data['smiles'][test_index].tolist()).SYBYL()), axis=1)
